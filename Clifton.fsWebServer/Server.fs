...
let startServer (websitePath:string) =
    ...
    listener (fun request response ->
        async {
            ...
            let path = request.RawUrl.LeftOf("?")           // Only the path, not any of the parameters
            let verb = request.HttpMethod                   // get, post, delete, etc.
            let parms = request.RawUrl.RightOf("?")         // Params on the URL itself follow the URL and are separated by a ?
            let kvParams = getKeyValues parms
            let responsePacket = route websitePath (path.ToLower()) (verb.ToLower()) kvParams
